...
iconsult_pathnames <- iconsult_raw$`Consulting Pathologist`
...
iconsult_raw$`Consulting Pathologist` <- 
...
path_namekey <- cpt_clean %>% 
  select(`PATHOLOGIST`) %>% 
  unique() %>%
  rename(Pathologist = PATHOLOGIST) %>% 
  mutate(Path_num = stri_rand_strings(24,4, pattern = "[0-9]"))
